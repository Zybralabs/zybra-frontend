...
it("formats Swap text input/output numbers correctly", () => {
  ...
  expect(formatNumber({ input: 7, type: NumberType.SwapTradeAmount, locale: "en-US" })).toBe(
    "7.00",
  );
...
it("formats NFT numbers correctly", () => {
  ...
  expect(formatNumber({ input: 0, type: NumberType.NFTTokenFloorPrice, locale: "en-US" })).toBe(
    "0",
  );
  ...
  expect(formatNumber({ input: 0, type: NumberType.NFTCollectionStats, locale: "en-US" })).toBe(
    "0",
  );
});
