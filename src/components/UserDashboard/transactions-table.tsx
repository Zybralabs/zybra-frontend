...
export function TransactionsComponent({ transactions }: TransactionsTableProps) {
  ...
  const getStatusStyle = (status: string) => {
    // Get colors from our centralized transaction status colors
    const colors = getTransactionStatusColors(status.toLowerCase());

    // Convert hex colors to tailwind-like classes
    // We're using a simplified approach with opacity for background
    return `bg-[${colors.bg}]/10 text-[${colors.text}]`;
  };
