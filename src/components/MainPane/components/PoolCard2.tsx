...
type PoolCardType = {
  ...
  investUrl?: string;
  apyData: {
    senior: InvestmentDataType;
    junior: InvestmentDataType;
  };
  tvl: number;
};
...
export default function PoolCard({
  ...
  return (
    <div className="bg-[#001C29] text-white p-6 rounded-xl w-2xl flex flex-col justify-between">
      ...
      <div className="flex w-full justify-between flex-col xl:flex-row gap-4 mb-8">
        ...
        <div className="xl:max-w-[66%] w-full">
          <div className="flex w-full justify-between mb-2">
            <div className="text-white text-md mb-0"></div>
            <div className="text-white lg:text-lg text-md mb-0 max-w-[60px] w-full text-right">
              APY
            </div>
            <div className="text-white lg:text-lg text-md mb-0 max-w-[110px] w-full">
              Min.Invesment
            </div>
          </div>
          {apyData?.junior && (
            <div className="flex w-full justify-between mb-0">
              <div className="text-white text-md mb-0 max-w-[50px] w-full text-sm xl:text-base">
                Junior.
              </div>
              <div className="text-white text-md mb-0 max-w-[50px] w-full text-sm xl:text-base">
                {apyData.junior?.apy}
              </div>
              <div className="flex gap-2 items-center text-white text-md mb-0 max-w-[110px] w-full text-sm xl:text-base">
                ${formatAmount(apyData.junior?.minInv)}{" "}
                <Link href={apyData.junior?.url} className="cursor-pointer text-xs text-gray-400">
                  target
                </Link>
              </div>
            </div>
          ...
          {apyData?.senior && (
            <div className="flex w-full text-sm xl:text-base justify-between mb-0">
              <div className="text-white text-md mb-0 max-w-[50px] w-full text-sm xl:text-base">
                Senior.
              </div>
              <div className="text-white text-md mb-0 max-w-[50px] w-full text-sm xl:text-base">
              ...
              <div className="flex gap-2 items-center text-white text-md mb-0 max-w-[110px] w-full text-sm xl:text-base">
                ${formatAmount(apyData.senior?.minInv)}{" "}
                <Link href={apyData.senior?.url} className="cursor-pointer text-xs text-gray-400">
                  target
                </Link>
              </div>
            </div>
