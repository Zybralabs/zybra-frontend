 ...
 */
export function useSingleCallResult(address: string, methodName: string, callInputs: any[]): CallStateResult {
  const { chainId, latestBlock } = useContext(BlockContext)
  const provider = useEthersProvider()
 ...
 */
export function useMainnetSingleCallResult(address: string, methodName: string, callInputs: any[]): CallStateResult {
